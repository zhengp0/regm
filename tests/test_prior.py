...
@pytest.mark.parametrize(
    ("mean", "sd", "size"),
    [(np.zeros(5), 1.0, None), (0.0, np.ones(5), None), (0.0, 1.0, 5)],
)
...
def test_spline_gaussian(order, domain_type, size, spline):
    prior = SplineGaussianPrior(
        mean=0.0, sd=1.0, order=order, size=size, domain_type=domain_type
    )
...
def test_spline_uniform(order, domain_type, size, spline):
    prior = SplineUniformPrior(
        lb=0.0, ub=1.0, order=order, size=size, domain_type=domain_type
    )
